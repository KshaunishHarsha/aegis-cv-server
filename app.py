...
def build_tf_model():
    """Build a lightweight CNN for artifact probability estimation."""
...
def detect_fake_probability(frame, model):
    """Infer fake probability using CNN."""
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
...
# =========================================
# Constants and Baselines (Pre-set)
# =========================================
BASELINE_SLOPE = -3.0
...
def analyze_video():
    ...
    # Aggregate results
    avg_slope = float(np.mean(spectral_slopes)) if spectral_slopes else -3.0
